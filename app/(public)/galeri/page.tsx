...
export default async function GaleriPage() {
  ...
  const galeri = (data || []) as unknown as GaleriRow[]

  return (
    <>
      <section className="relative bg-mesh-sunset py-16 md:py-20 -mt-16 md:-mt-20 pt-28 md:pt-32 overflow-hidden">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-10 right-20 w-72 h-72 bg-orange-400 rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-3xl opacity-20 animate-blob" />
        </div>
        ...
        <div className="container mx-auto px-4 relative">
          <div className="max-w-3xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 bg-white/70 dark:bg-stone-900/70 backdrop-blur-md border border-orange-200 dark:border-orange-900/50 rounded-full px-4 py-1.5 mb-6 shadow-sm">
              ...
              <span className="text-xs font-semibold text-stone-700 dark:text-stone-300">Galeri Foto</span>
            </div>
            <h1 className="text-4xl md:text-6xl font-display font-extrabold mb-4 leading-tight">
              ...
              <br />
              <span className="text-stone-900 dark:text-stone-50">Bersama Kami</span>
            </h1>
      ...
      <section className="container mx-auto px-4 py-12 md:py-16">
        {galeri.length === 0 ? (
          <div className="max-w-md mx-auto text-center py-12">
            <ImageIcon className="h-16 w-16 mx-auto text-stone-300 dark:text-stone-600 mb-4" />
            <p className="text-stone-500 dark:text-stone-400">Belum ada foto di galeri.</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
            {galeri.map((foto) => (
              <div
                key={foto.id}
                className="group relative aspect-square overflow-hidden rounded-2xl bg-stone-200 dark:bg-stone-800 shadow-md hover:shadow-2xl hover:shadow-orange-500/20 transition-all duration-500 hover:-translate-y-1"
              >
                <Image
                  src={foto.foto_url}
