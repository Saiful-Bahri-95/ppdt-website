  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? 'glass shadow-lg shadow-orange-500/10 border-b border-orange-200/50'
          : 'bg-transparent'
      )}
    >
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col leading-tight">
              ...
              <span className="text-[10px] md:text-xs text-stone-600 hidden sm:block font-medium">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {navigationLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="relative px-4 py-2 text-sm font-semibold text-stone-700 hover:text-orange-600 rounded-full transition-all duration-300 hover:bg-orange-50 group"
              >
                {link.label}
          ...
          >
            {mobileMenuOpen ? <X className="h-5 w-5 text-orange-600" /> : <Menu className="h-5 w-5 text-stone-700" />}
          </Button>
        </div>
        ...
        >
          <div className="flex flex-col gap-1 pt-2 border-t border-orange-200/50">
            {navigationLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                onClick={() => setMobileMenuOpen(false)}
                className="px-4 py-3 text-sm font-semibold text-stone-700 hover:text-orange-600 hover:bg-orange-50 rounded-xl transition-all duration-200"
              >
                {link.label}
              </Link>
            ))}
