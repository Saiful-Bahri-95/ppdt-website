  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? 'glass shadow-lg shadow-orange-500/10 border-b border-orange-900/30'
          : 'bg-transparent'
      )}
    >
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col leading-tight">
              ...
              <span className="text-[10px] md:text-xs text-stone-400 hidden sm:block font-medium">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {navigationLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="relative px-4 py-2 text-sm font-semibold text-stone-300 hover:text-orange-400 rounded-full transition-all duration-300 hover:bg-orange-950/50 group"
              >
                {link.label}
          ...
          <Button
            variant="ghost"
            size="icon"
            className="lg:hidden hover:bg-orange-950/50"
        ...
        >
          <div className="flex flex-col gap-1 pt-2 border-t border-orange-900/30">
            {navigationLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                onClick={() => setMobileMenuOpen(false)}
                className="px-4 py-3 text-sm font-semibold text-stone-300 hover:text-orange-400 hover:bg-orange-950/50 rounded-xl transition-all duration-200"
              >
                {link.label}
              </Link>
            ))}
